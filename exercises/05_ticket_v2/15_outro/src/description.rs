...
impl TryFrom<&str> for TicketDescription {
    ...
    fn try_from(value: &str) -> Result<Self, Self::Error> {
        if value.len() > 500 {
            return Err(DescriptionError::DescriptionTooLong("The description cannot be longer than 500 bytes".to_string()))
        } else if value.is_empty() {
            return Err(DescriptionError::DescriptionCannotBeEmpty("The description cannot be empty".to_string()))
        }
