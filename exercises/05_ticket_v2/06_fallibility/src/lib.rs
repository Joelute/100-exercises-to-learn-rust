...
impl Ticket {
    pub fn new(title: String, description: String, status: Status) -> Result<Ticket, String> {
        ...
        if title.len() > 50 {
            return Err(String::from("Title cannot be longer than 50 bytes".to_string()));
        }
        if description.is_empty() {
            return Err(String::from("Description cannot be empty".to_string()));
        }
        if description.len() > 500 {
            return Err(String::from("Description cannot be longer than 500 bytes".to_string()));
        }
